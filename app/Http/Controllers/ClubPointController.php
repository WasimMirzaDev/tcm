...
class ClubPointController extends Controller
{
    ...
    public function savethetoken(Request $request)
    {
        $club_point = new ClubPoint;
        $club_point->user_id = $request->user_id;
        $club_point->points = $request->token;
        $club_point->order_id = $request->event_id;
        $club_point->convert_status = 0;
        $club_point->save();
        Artisan::call('cache:clear');
        
        flash(translate('User token has been updated successfully'))->success();
