...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('Convert Token To Wallet')}}</h5>
                </div>
                <div class="card-body">
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('Give the Token To User')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('savethetoken') }}" method="POST">
                        @csrf
                        
                        <div class="form-group row">
                            <div class="col-lg-4">
                                <label class="col-from-label">{{translate('Set Token For ')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <input type="number" min="0" step="0.01" class="form-control" name="token" value="" placeholder="100" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-lg-4">
                                <label class="col-from-label">{{translate('Select Event')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <select class="form-control" name="event_id" required>
                                    
                            <option >Select the Event</option>
                              @foreach (\App\Models\Event::all() as $key => $event)
                        ...
                        <div class="form-group row">
                            <div class="col-lg-4">
                                <label class="col-from-label">{{translate('Select User')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <select class="form-control" name="user_id" required>
                                    
                            <option >Select the User</option>
                              @foreach (\App\Models\User::where('user_type','customer')->get() as $key => $event)
